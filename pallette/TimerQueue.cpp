...
TimerQueue::~TimerQueue()
{
    ...
    for (auto freeTimer : freeTimers_)
    {
        delete freeTimer;
    }
}
...
void TimerQueue::handleRead()
{
    ...
    detail::readTimerfd(timerfd_, now);//读取timerfd，避免重复唤醒

    getExpired(now);//得到now时间响应的定时器

    callingExpiredTimers_ = true;
    cancelingTimers_.clear();//清空待删除定时器列表
...
void TimerQueue::reset(Timestamp now)
{
    ...
    for (auto expireTimer : activeTimers_)
    {
        ...
        {
            freeTimers_.push_back(expireTimer);
        }
    }
