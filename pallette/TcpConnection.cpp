...
> Created Time: 2019年01月30日 星期三 10时25分24秒
************************************************************************/
...
#include <pallette/SocketsOperations.h>
#include <pallette/WeakCallback.h>
...
void pallette::defaultConnectionCallback(const TcpConnectionPtr& conn)
{
...
void pallette::defaultMessageCallback(const TcpConnectionPtr&, Buffer* buffer, Timestamp)
{
    buffer->retrieveAll();
}
...
void TcpConnection::sendInLoop(const void* data, size_t len)
{
    ...
    if (!channel_->isWriting() && outputBuffer_.readableBytes() == 0)
    {
        ...
        if (nwrote >= 0)
        {
            ...
            {
                bytesSent_ += static_cast<uint64_t>(nwrote);
